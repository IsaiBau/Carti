...
const CrearPersona: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { nombre, sexo, fecha_nac, curp, rfc,password, id_tipo_persona } = formData;

    // Validación de campos obligatorios
    if (!nombre || sexo === undefined || !fecha_nac || !curp || !rfc || !password || !id_tipo_persona) {
    ...
    if (!noCaracteresEspeciales.test(curp)) {
        setErrorMsg('La CURP solo debe contener letras mayúsculas y números, sin caracteres especiales.');
        setLoading(false);
        return;
    }
    ...
    if (!noCaracteresEspeciales.test(rfc)) {
        setErrorMsg('El RFC solo debe contener letras mayúsculas y números, sin caracteres especiales.');
        setLoading(false);
        return;
    }
        ...
        <div className='flex flex-col justify-center items-center text-center w-[50%] p-20'>
          ...
          <div className='flex flex-col w-md'>
            ...
            <div className='my-2 grid gap-4 grid-cols-2 mt-5'>
                ...
                <p className='poppins-regular text-[#8B8B8B] text-sm'>¿Ya tienes una cuenta? <Link className='poppins-regular text-[#2787E0] text-sm cursor-pointer' to="/login">Ingresa ahora</Link></p>
              </div>
            </form>
          </div>
        </div>
